...
import { isValidIPOrCIDR, getNetworkType, formatIPForDisplay } from '@/lib/ipUtils';
...
export const IPAccessModal: React.FC<IPAccessModalProps> = ({
  ...
  const [networkPolicy, setNetworkPolicy] = useState<'both' | 'lan' | 'wan'>('both');
  const [ipAccessPolicy, setIpAccessPolicy] = useState<'all' | 'restricted'>('all');
  ...
  const handleAddIP = () => {
    ...
    if (!validateIP(trimmedIP)) {
      setIpError('Please enter a valid IP address or CIDR range (e.g., 192.168.1.1 or 192.168.1.0/24)');
      return;
    }
  ...
  const handleSave = () => {
    ...
    if (ipAccessPolicy === 'restricted' && allowedIPs.length === 0) {
      setIpError('Please add at least one IP address when restricting access by IP');
      return;
    }
  ...
  const getNetworkIcon = (type: string) => {
    switch (type) {
      ...
      case 'lan': return <Wifi className="w-4 h-4" />;
      case 'wan': return <Router className="w-4 h-4" />;
  ...
  const getNetworkDescription = (type: string) => {
    switch (type) {
      case 'both': return 'Allow streaming from both local network and internet';
      case 'lan': return 'Only allow streaming from local network (same subnet)';
      case 'wan': return 'Only allow streaming from internet (remote access)';
      default: return '';
    }
  };
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Configure network and IP-based access restrictions for {user.username || user.userId}
          </DialogDescription>
        </DialogHeader>
                ...
                >
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="font-medium text-sm capitalize">
                      {type === 'both' ? 'Both (LAN + WAN)' : type.toUpperCase()}
                    </span>
                    {networkPolicy === type && (
              ...
              {(['all', 'restricted'] as const).map((type) => (
                ...
                >
                  ...
                  <p className="text-xs text-muted-foreground">
                    ...
                    {type === 'restricted' && 'Allow only specific IP addresses or CIDR ranges'}
                  </p>
                </button>
              ))}
            </div>
                ...
                {allowedIPs.length > 0 && (
                  <div className="space-y-2">
                    <Label className="text-xs text-muted-foreground">
                      {allowedIPs.length} IP address{allowedIPs.length !== 1 ? 'es' : ''} configured
                    </Label>
                ...
                {ipAccessPolicy === 'restricted' && allowedIPs.length === 0 && (
                  <div className="flex items-center gap-2 py-4 px-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
                    ...
                    <div className="text-left text-yellow-800 dark:text-yellow-200 text-xs">
                      <strong>Warning:</strong> No IP addresses configured. Add at least one IP address to restrict access, otherwise all access will be blocked.
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
              ...
              <div className="flex flex-wrap gap-1">
                {userDevices
                  .filter(device => device.ipAddress)
                  .map((device, index) => {
                    const networkType = device.ipAddress ? getNetworkType(device.ipAddress) : 'unknown';
                    return (
                      <Badge key={index} variant="outline" className="text-xs bg-blue-100 dark:bg-blue-800/50">
                        {device.deviceName || device.deviceIdentifier}: {device.ipAddress} ({networkType.toUpperCase()})
                      </Badge>
                    );
                  })}
              </div>
        ...
        <div className="flex justify-end gap-2 pt-4 border-t">
          ...
          <Button 
            onClick={handleSave}
            disabled={ipAccessPolicy === 'restricted' && allowedIPs.length === 0}
          >
            Save Policies
          </Button>
        </div>
